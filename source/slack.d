...
struct Response {
    ...
    auto opCast(T : bool)() const {
        if (_task) {
            assert(_task == null, "must await() asynchronous Response");
            return false;
        }
...
package:
    ...
    this(Task!(post, string, string[string], HTTP)* task) {
        _task = task;
        taskPool.put(_task);
    }
private:
    JSONValue _value;
    Task!(post, string, string[string], HTTP) *_task;
}
...
struct Slack {
    ...
    Response postMessage(string msgString, bool async = false) const {
        ...
        return async ?
            Response(task!post(slackApiUrl ~ "chat.postMessage", data, http)) :
            Response(post(slackApiUrl ~ "chat.postMessage", data, http));
    }    // postMessage()
    ...
    Response conversationsHistory(string channelId,
                                  SysTime oldest = SysTime(),
                                  SysTime latest = SysTime(),
                                  bool async = false) const
    {
        ...
        http.addRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        return async ?
            Response(task!post(slackApiUrl ~ "conversations.history", data, http)) :
            Response(post(slackApiUrl ~ "conversations.history", data, http));
    }
    ...
    Response conversationsList(bool async = false) const {
        ...
        http.addRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        return async ?
            Response(task!post(slackApiUrl ~ "conversations.list",
                        ["token": _token, "exclude_archived": "true"], http)) :
            Response(post(slackApiUrl ~ "conversations.list",
                        ["token": _token, "exclude_archived": "true"], http));
    }
